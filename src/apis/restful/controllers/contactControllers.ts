...
export default class ContactControllers{
    ...
    static async create(req: NextApiRequest,res: NextApiResponse){
        try {
            ...
            await ContactService.create({name,email,message}).then((rslt)=>{
    ...
    static async getAll(req: NextApiRequest,res: NextApiResponse){
        try {
            await ContactService.findAll().then((rslt)=>{
    ...
    static async getOne(req: NextApiRequest,res: NextApiResponse){
        try {
            ...
            await ContactService.findByPk(contact_id).then((rslt)=>{
                if(!rslt){
                    return Response.success(res,200,{
    ...
    static async delete(req: NextApiRequest,res: NextApiResponse){
        try {
            const {contact_id}= req.query;
            const isContactExist = await ContactService.findByPk(contact_id);
            if (!isContactExist){
                return Response.error(res,404,{
                    message:"This contact is not in system"
                })
            }
            await ContactService.destroy({id:contact_id}).then((rslt)=>{
                return Response.success(res,200,{
